...
fn main() {
    ...
    let ngap_data =
        "0015404A000004001B00084002F898000000000052400F06004D79206C69\
        74746C6520674E420066001F01000000000002F898000100080080000001\
        0002F8390001001881C00013880015400140";

    let ngap_data = hex::decode(ngap_data)
        .expect("Unable to parse data as hex bytes");
    println!("{:#x?}", ngap_data);

    let buff: *const libc::c_void =
        ngap_data.as_ptr().cast::<libc::c_void>();
    ...
    unsafe {
        ...
        let rval = ngap_codec::aper_decode(
            ctx, &pdutype, &mut s, buff, len, 0, 0,
        );
        println!("{:?}", rval);
    }
}
